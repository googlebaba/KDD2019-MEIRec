...
class Model(object):
    ...
    def create_model_graph(self):
        ...
        def _myCNN( _X_S, hidden_size, name):
            with tf.variable_scope(name):
                x_input = tf.reshape(_X_S, [-1, 5, 64, 1])
                conv1 = tf.layers.conv2d(x_input, filters = 12, kernel_size=[2, 64], padding = 'valid', activation=tf.nn.relu)
                pool1 = tf.layers.max_pooling2d(inputs=conv1, pool_size = [2,2], padding='SAME', strides=2)
                pool1_flat = tf.reshape(pool1, [-1, 2*1*12])
                hidden_layer_cnn = tf.layers.dense(inputs=pool1_flat, units=hidden_size, activation=tf.nn.relu)
                _O = tf.transpose(hidden_layer_cnn)
                return _O
...
#             self.user_query_item_term_lstm_output = tf.transpose( tf.reduce_mean( user_item_seq_term_embed_list, 0 ) ) #取均值
            ...
            self.user_query_item_term_lstm_output = _myRNN(user_item_seq_term_embed_list, 64, "query_lstm_term")
        ...
        with tf.variable_scope("query_item_query_embedding"):
            query_item_term_embed_list = []

            raw_step_termids_list = tf.split( self.query_item_query_feat[-10 * 5:],  [10] * 5, 0 )  
            for i in range( len( raw_step_termids_list ) ):
                ...
                query_item_term_embed_list.append( step_avg_embedding  )
            
            print( "query_item_query_term_embed_list", len(query_item_term_embed_list) )
#             self.user_query_item_term_lstm_output = tf.transpose( tf.reduce_mean( user_item_seq_term_embed_list, 0 ) ) #取均值
            
            
            self.query_item_query_cnn_output = _myCNN(query_item_term_embed_list, 64, "query_cnn_term")
            
            print("cnn self.query_item_query_cnn_output shape", self.query_item_query_cnn_output.get_shape())  #64 * batch_size
        
        with tf.variable_scope("query_user_item_embedding"):
            query_user_term_embed_list = []

            raw_step_termids_list = tf.split( self.query_user_item_feat[-10 * 5:],  [10] * 5, 0 )  
            for i in range( len( raw_step_termids_list ) ):
                item_terms = raw_step_termids_list[ i ][:5]
                ...
                query_user_term_embed_list.append( step_avg_embedding  )
            
            print( "query_user_term_embed_list", len(query_user_term_embed_list) )
#             self.user_query_item_term_lstm_output = tf.transpose( tf.reduce_mean( user_item_seq_term_embed_list, 0 ) ) #取均值
            
            
            self.query_user_item_cnn_output = _myCNN(query_user_term_embed_list, 64, "item_cnn_term")
